...
#define FAT_MAX_COV_DIR_SIZE 128
...
u32 *fat_dir;
u32 fat_cov_dir_buff[FAT_MAX_COV_DIR_SIZE];
u32 *fat_cov_dir;
File file;
    ...
    for (;;) {
        ...
        for (u = 0; u < fat_cluster_size; u++) {
            ...
            for (i = 0; i < 16; i++, ptr += 32) {
                ...
                if (*ptr == 0)return fat_filter_cover_records();
                ...
                fat_dir_size++;
                lfn_record = 0;
                if (fat_dir_size == OS_MAX_DIR_SIZE)return fat_filter_cover_records();
            }
...
u8 fat_filter_cover_records() {
    ...
    for (i = 0; i < fat_dir_size; i++) {
        ...
        if (!rec->is_dir && str_extension(".cov", rec->name)) {
            if (fat_cov_dir_size < FAT_MAX_COV_DIR_SIZE) {
                fat_cov_dir[fat_cov_dir_size] = fat_dir[i];
                fat_cov_dir_size++;
            }
        } else {
            fat_dir[visible_size] = fat_dir[i];
            visible_size++;
        }
    }

    fat_dir_size = visible_size;
    osReleaseRam(sizeof (FatFullRecord));
    return 0;
}
