...
u8 osInit() {
    ...
    gCleanScreen();
    gDrawStringCx("Sistema iniciando...", 12);
    gRepaint();
...
u8 osRomMenu(FatFullRecord *rec) {
    ...
    const u8 * rom_menu_str[] = {
        "Menu de arquivos", "Carregar e iniciar", "Apenas carregar", "Visualizacao hexadecimal", 0
    };
...
u8 osMainMenu() {
    ...
    u8 selector;
    const u8 * menu_str[] = {
        "Menu principal", "Iniciar jogo", "Informacoes do dispositivo", "Sobre", 0
    };
...
void osSysInfo() {
    ...
    gDrawString("Versao da CPLD: ", x, y);
    gAppendNum(bi_get_cpld_ver());
    y += 2;

    gDrawString("Versao do SO: ", x, y);
    gAppendNum(os_ver);
    y += 2;

    gDrawString("Max. Diretorios: ", x, y);
    gAppendNum(OS_MAX_DIR_SIZE);
    y += 2;

    gDrawString("Tipo de Cartucho: ", x, y);
    if (bi_is_gg_cart()) {
        gAppendString("GG");
    } else {
        gAppendString("SMS");
    ...
    gDrawString("Dat. Firmware: 09/06/2026", x, y);
    y += 2;
    bi_set_rom_bank(1);
...
void osAbout() {
    ...
    if (gg_cart) {
        ...
        gConsPrint("Firmware desenvolvida por:");
        gConsPrint("@tavinho.games");
		gConsPrint("Agradecimentos: thalescamara e ludufre");
    }
    ...
    gConsPrint("Suporte:");
    gSetPal(0);
    gConsPrint("Instagram:");
    gConsPrint("tavinho.games");

    gConsPrint("");
    gSetPal(4);
    gConsPrint("Controles:");
    gSetPal(0);
    if (gg_cart) {
        gConsPrint("Esq/Dir - Alternar pagina");
    } else {
        gConsPrint("Esq/Dir - Alternar pagina");
    }
    gConsPrint("Bot. 1 - Menu de arquivos");
    gConsPrint("Bot. 2 - Voltar");
    gConsPrint("Bot. 2 na raiz - Menu principal");
...
u8 osSelectGame(FatFullRecord *rec) {
    ...
    gConsPrint("");
    if (bi_is_gg_cart()) {
        gConsPrint("!!NAO DESLIGUE!!");
        gSetPal(0);
        gConsPrint("-------------------");
    } else {
        gConsPrint("!NAO DESLIGUE O CONSOLE!");
        gSetPal(0);
        gConsPrint("----------------------------");
    }
    gRepaint();
...
u8 _osSaveSram(FatFullRecord *rec) {
    ...
    gConsPrint("Salvando dados da RAM...");
    gRepaint();
...
u8 osUpdateMenu(FatFullRecord *rec) {
    ...
    u8 resp;
    const u8 * menu_str[] = {
        "Menu de arquivos", "Cancelar", "Atualizar firmware", 0
    };
...
u8 osUpdate(FatFullRecord *rec) {
    ...
    gCleanScreen();
    gSetPal(0);
    gDrawStringCx("!NAO DESLIGUE O CONSOLE!", 11);
    gRepaint();
